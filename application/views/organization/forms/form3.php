<div class="span-19 last" id="content_main">
	...
	<? foreach($officers as $officer): ?>
		<tr>
		    <td><? if($officer['filepath'] != '' || $officer['filepath'] != null): ?><a href="<?= $base_url.'uploads/'.$officer['filepath']?>"><img src="<?= $base_url.'uploads/'.$officer['filepath'] ?>"/></a><? else: ?>N/A<? endif; ?></td>
			<td><?= $officer['webmail'] ?></td>
			<td><?= $officer['email'] ?></td>
			<td><?= $officer['position'] ?></td>
            <td><? $this->load->model('student_model');
                   $orgs = $this->student_model->get_organizations($officer['studentid'], $appsemid);
                   if(count($orgs) > 1){
                       foreach($orgs as $org){
                           if($org['organizationid'] != $orgid){
                                if($this->session->user_group_is(OSA_GROUPID)){
                                    echo '<span style="font-size:8px;">'.anchor('osa/view_application/'.$org['organizationid'].'/'.$appsemid, $org['orgname']).'</span><br/><br/>';
                                }else{
                                    echo $org['orgname'].'<br/><br/>';
                                }
                           }
	...
	<? foreach($members as $member): ?>
		<tr>
		    <td><? if($member['filepath'] != '' || $member['filepath'] != null): ?><a href="<?= $base_url.'uploads/'.$member['filepath']?>"><img src="<?= $base_url.'uploads/'.$member['filepath'] ?>"/></a><? else: ?>N/A<? endif; ?></td>
			<td><?= $member['webmail'] ?></td>
			<td><?= $member['email'] ?></td>
            <td><? $this->load->model('student_model');
                   $orgs = $this->student_model->get_organizations($member['studentid'], $appsemid);
                   if(count($orgs) > 1){
                       foreach($orgs as $org){
                           if($org['organizationid'] != $orgid){
                                if($this->session->user_group_is(OSA_GROUPID)){
